...
- Scans `/** ... */` JSDoc comments in `.js`/`.jsx`/`.ts`/`.tsx` files for ` ```mermaid ` fenced code blocks — on blob (source file) pages, and on PR "Files changed" / commit diff pages
...
1. Open any `.js`/`.jsx`/`.ts`/`.tsx` file on GitHub — either the "blob" view (e.g. `github.com/org/repo/blob/main/src/foo.ts`) or a PR's "Files changed" tab / a commit diff
2. Scroll to a JSDoc comment containing a ` ```mermaid ` fenced code block
